...
## INTAKE_COMPLETE format (from SYSTEM_PROMPT) — 18 sections, v2
Field order matters — parseIntakeComplete() uses boundary-based regex. FIELDS array must exactly match these labels:
```
COMPANY CONTEXT / THE ROLE / SENIORITY & OWNERSHIP / REPORTING STRUCTURE / CANDIDATE PROFILE /
REQUIREMENTS / WORK STYLE & CULTURE / COMPENSATION / TIMELINE / INTERVIEW PROCESS /
DEAL BREAKERS / SELL POINTS / PAST HIRING SIGNAL / SUCCESS METRICS / VALIDATED ASSUMPTIONS /
CONTACT / OPEN FLAGS / RECRUITER NOTES
```
HiringBrief interface fields (in order): companyContext, role, seniorityOwnership, reportingStructure, candidateProfile, requirements, workStyleCulture, compensation, timeline, interviewProcess, dealBreakers, sellPoints, pastHiringSignal, successMetrics, validatedAssumptions, contact, openFlags, recruiterNotes.
CONTACT field is structured multi-line: "Name: X\nTitle: Y\nEmail: Z\nCompany: A\nWebsite: B".
Old field names (budget, notableQuotes, openQuestions) are gone — do not use them.
