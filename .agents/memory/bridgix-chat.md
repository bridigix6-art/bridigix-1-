...
AI outputs this block and nothing else when intake is done. Field order matters — parseIntakeComplete() uses boundary-based regex (each field ends where the next label starts):
```
INTAKE_COMPLETE
COMPANY CONTEXT: ...
ROLE: ...
SENIORITY & OWNERSHIP: ...
REPORTING STRUCTURE: ... (in system prompt but not in HiringBrief — gets captured as part of SENIORITY & OWNERSHIP by regex)
PAST HIRING SIGNAL: ...
WORK STYLE & CULTURE: ...
REQUIREMENTS: ...
TIMELINE: ...
...
RECRUITER NOTES: ... (added — AI's stated assumptions/inferences during conversation)
```
HiringBrief interface has recruiterNotes field. buildSidebarFields and BRIEF_REVIEW_FIELDS both include it (conditionally hidden if empty).
